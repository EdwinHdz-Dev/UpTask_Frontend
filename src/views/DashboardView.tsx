        ...
        <>
            ...
            {data.length ? (
                <ul role="list" className="divide-y divide-gray-100 border border-gray-100 mt-10 bg-white shadow-lg">
                    {data.map((project) => {
                        const progress = Math.round(calculateTheoreticalProgress(project.createdAt, project.estimatedCompletionDate));
                        const isTimeUp = progress > 100;
                        ...
                        return (
                            <li key={project._id} className="flex justify-between gap-x-6 px-5 py-10">
                                ...
                                <div style={{ width: '150px', height: '150px' }}>
                                    <CircularProgressbar
                                        value={isTimeUp ? 100 : progress}
                                        text={isTimeUp ? 'Tiempo terminado' : `${progress}%`}
                                        styles={buildStyles({
                                            textSize: '10px',
                                            textColor: isTimeUp ? '#000000' : '#3B82F6',
                                            pathColor: isTimeUp ? 'rgb(220 38 38 / var(--tw-bg-opacity))' : '#3B82F6',
                                            trailColor: ''
                                        })}
                                    />
                                </div>
